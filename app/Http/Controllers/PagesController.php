...
class PagesController extends Controller
{
    ...
    {
        if (isset($_GET['search']) && isset($_GET['date']) && isset($_GET['group_type']))
        {
            $search =   date('Y-m-d', strtotime($request->date));
            ...
            return view('pages.history',['posts'=>$posts])->withQuery ( $pagination )->render();
        }
            ...
            $posts = BufferPosting::with('groupInfo.socialGroup','accountInfo.user')->where('post_text', 'like', '%' . $search . '%')
                ->orWhereIn('group_id', function($query) use ($search)
                {
                    $query->select('id')
                        ...
                        ->orWhere('social_post_groups.type', 'like', '%' . $search . '%');
                })
        ...
        elseif ($request->has('dataFilter'))
        {
            ...
            if ($search == 'all')
            {
                $post = BufferPosting::with('groupInfo.socialGroup','accountInfo.user')->paginate(10);
        ...
        {
            $search =   date('Y-m-d', strtotime($request->dateTime));
            $posts = BufferPosting::with('groupInfo.socialGroup','accountInfo.user')
                ->where('created_at', 'like', '%' . $search . '%')
                ->paginate(5);
            $pagination = $posts->appends ( array (
            ...
            $data = view('pages.date',['posts'=>$posts])->withQuery ( $pagination )->render();
            return $data;
            //return view('pages.history',compact('posts'))->withQuery ( $pagination );
